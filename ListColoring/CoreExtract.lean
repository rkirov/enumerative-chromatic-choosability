...
* **A connected `2`-regular graph is a cycle.** `ListColoring.exists_iso_closePath_of_two_regular`:
  such a graph is isomorphic to `ListColoring.closePath k` for some `k ≥ 2`, i.e. to the cycle on
  `k + 1` vertices. The proof takes any cycle walk (which exists by
  `ListColoring.exists_isCycle_of_two_le_degree`), observes that its vertices already carry two
...
* **A cycle plus a chord path is a theta graph.**
  `ListColoring.nonempty_iso_thetaGen_of_paths`: a graph that is exactly the union of three internally
  disjoint paths of lengths `a`, `b`, `c` joining two distinct vertices is `ListColoring.thetaGen a b
  ...
  `ListColoring.theta m`, by `ListColoring.nonempty_iso_theta_of_iso_thetaGen`.
...
number rather than by a nest of `Option`s. `ListColoring.pathNum k` numbers `PathV k` from
`pathEnd k = 0` up to `pathStart k = k`, `ListColoring.pathAt k` inverts it, and

* `ListColoring.pathG_adj_num` : `pathG k` joins consecutive numbers;
* `ListColoring.closePath_adj_num` : `closePath k` joins consecutive numbers and also `0` to `k`.
...
* `ListColoring.pathNum`, `ListColoring.pathAt` : the numbering of `PathV k` and its inverse
* `ListColoring.HasCore` : `G` has a core — the conclusion of `ListColoring.hasCore`
* `ListColoring.armIdx`, `ListColoring.tgArm`, `ListColoring.tgArmLen`, `ListColoring.ThetaStep` :
  the arms of `thetaGen a b c`, on indices
* `ListColoring.IsThetaArms` : `H` is exactly the union of three internally disjoint paths
* `ListColoring.armVertex` : the vertex of `H` at a given index of the `thetaGen` layout

## Main results

* `ListColoring.addPendantIso` : isomorphisms lift through `SimpleGraph.addPendant`
* `ListColoring.isoAddPendantOfDegreeOne` : a graph with a vertex of degree one is a pendant
  attachment to the graph with that vertex removed
* `ListColoring.pathG_adj_num`, `ListColoring.closePath_adj_num` : path and cycle adjacency, on
  indices
* `ListColoring.hasCore` : **every connected finite graph has a core**
* `ListColoring.nonempty_iso_closePath_of_index` : a cyclic indexing of the vertices of a graph, in
  which adjacency is exactly cyclic adjacency of indices, is an isomorphism onto `closePath k`
* `ListColoring.exists_iso_closePath_of_two_regular` : **a connected `2`-regular graph is a cycle**
* `ListColoring.armStepB_iff_armIdx`, `ListColoring.thetaGen_adj_iff` : the edges of
  `thetaGen a b c` are exactly the steps along its three arms, and nothing else
* `ListColoring.nonempty_iso_thetaGen_of_index`, `ListColoring.nonempty_iso_thetaGen_of_arms`,
  `ListColoring.nonempty_iso_thetaGen_of_paths` : **a graph presented as three internally disjoint
  paths between two branch vertices is a theta graph**, in three increasingly explicit forms
* `ListColoring.nonempty_iso_thetaOf_thetaGen`, `ListColoring.nonempty_iso_theta_thetaGen`,
  `ListColoring.nonempty_iso_theta_of_iso_thetaGen` : the shape `(2, 2, 2m)`, moved onto
  `ListColoring.theta m`
-/
...
`PathV k` is `Unit` wrapped in `k` copies of `Option`; `ListColoring.pathNum` turns such a vertex
into its distance from `pathEnd k`. Because `PathV (k+1)` and `Option (PathV k)` are definitionally
but not syntactically equal, everything below is written by the equation compiler or crossed with
...
/-- The vertex of `pathG k` at index `i`; the inverse of `ListColoring.pathNum` on `i ≤ k`. -/
...
/-- **A connected `2`-regular graph is a cycle.** It is isomorphic to `ListColoring.closePath k` —
the cycle on `k + 1` vertices — for some `k ≥ 2`.

Take any cycle walk, which exists because the minimum degree is at least `2`
(`ListColoring.exists_isCycle_of_two_le_degree`). Each of its vertices already has two neighbours on
...
`ListColoring.thetaGen a b c` is laid out on `Fin (a+b+c-1)` by an index formula
(`ListColoring.thetaGenAdjB`), which says nothing directly about arms. The block below turns that
...
`ListColoring.armIdx o len ps pt i` is the index of the vertex at position `i` along an arm of
length `len` whose interior vertices are `o, o+1, …, o+len-2` and whose ends are the branch vertices
`ps` and `pt`. The three arms of `thetaGen a b c` are this with

* `o = 0`, `len = a`; `o = a-1`, `len = b`; `o = a+b-2`, `len = c`,

and `ps = a+b+c-3`, `pt = a+b+c-2`. That reading of the layout — `ListColoring.thetaGen_adj_iff`
— is **checked by `decide` at every shape with `1 ≤ a, b, c ≤ 4`**, and at `(2,2,6)` and
...
/-- The converse of `ListColoring.thetaArms_rigid`. -/
lemma thetaArms_rigid' (h : IsThetaArms H a b c p q r) {t t' : ℕ} (ht : t < 3) (ht' : t' < 3)
    {i i' : ℕ} (hi : i ≤ tgArmLen a b c t) (hi' : i' ≤ tgArmLen a b c t')
...
comment when it writes "`C₁ ∪ P₁ = θ_{a,b,c}`": a cycle `C₁` together with a path `P₁` joining
two of its vertices and otherwise disjoint from it *is* three internally disjoint paths — the two
arcs of `C₁` and `P₁` — between the two ends of `P₁`. Not novel; the content here is the
identification of the two encodings. See `ListColoring.nonempty_iso_thetaGen_of_paths` for the
same statement with the hypotheses spelt out one path at a time. -/
...
/-! ### The shape `(2, 2, 2m)`: `ListColoring.theta m`

Rubin's theorem names its theta family `θ_{2,2,2m}`, and this development builds that family
separately, as `ListColoring.theta m` — a path of length `2m` with two extra vertices, each joined
to both of its ends (`ListColoring.thetaOf`). The two models therefore have to be identified. Rather
than compare index formulas, `ListColoring.thetaOf k` is presented as three explicit arms of lengths
`2`, `2` and `k` and handed to `ListColoring.nonempty_iso_thetaGen_of_paths`. -/
...
/-- **`ListColoring.thetaOf k` is `θ_{2,2,k}`.** The path of length `k` with two extra vertices each
...
/-- **`ListColoring.theta m` is `θ_{2,2,2m}` on the general model.** Bookkeeping between the two
...
isomorphic to `ListColoring.theta m`, which is the form `ListColoring.CoreIsTheta` — and hence
`ListColoring.RubinTheorem` — asks for. Bookkeeping; the family is **Rubin's**
...
/-- `ListColoring.ThetaStep` is a bounded existential, hence decidable — which is what lets the
...
-- `ListColoring.thetaGen_adj_iff`, checked at every shape with `1 ≤ a, b, c ≤ 4`
