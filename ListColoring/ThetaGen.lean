...
`ListColoring.RubinHard` builds the three-arm case `ListColoring.thetaGen a b c`; this file builds
`ListColoring.gtheta ks` for a list `ks` of arm lengths of any length, and classifies which of them
are `2`-choosable:
...
can be fed into the proof of Rubin's theorem without circularity — `ListColoring.RubinHard` consumes
the three-arm case as `ListColoring.ThetaClassification`. Nothing here may therefore appeal to
Rubin's theorem, and nothing does.

The other half of Rubin's hard direction is a structural statement about connected graphs of minimum
degree `≥ 2` that are not cycles, and **no correct form of it is formalized here**. Two candidate
forms have already been refuted in this repository, both of them its own inventions and neither the
literature's; `ListColoring.Rubin` and the section "A removed definition, and why it must stay
removed" in `ListColoring.RubinHard` record the refutations, and `plan.md` records what a correct
statement would need. Neither should be reintroduced.
...
(`ListColoring.not_choosable_two_K24`). Three-arm thetas alone therefore do not suffice, which is
...
vertices. The layout matches `ListColoring.thetaGen` exactly when `ks = [a, b, c]`, which is why
`ListColoring.gAdjB_triple` is an equality of Booleans rather than an isomorphism.
...
(`ListColoring.ArmBlocks`). If every one of the four pairs is blocked by some arm, there is no
coloring at all (`ListColoring.gcol_gBadLists_eq_zero`). Two gadgets suffice:
...
* an arm of length `≥ 2` blocks any single prescribed pair `(α, β)` with `α ≠ β`, through the
  forced chain `ListColoring.gArmLists`.
...
* `ListColoring.gtheta` : the generalized theta graph `Θ(k₁, …, k_n)`
* `ListColoring.ArmBlocks` : an arm's lists block a pair of branch colors
* `ListColoring.gArmLists`, `ListColoring.roleA`, `ListColoring.roleB`, `ListColoring.gBadLists` :
  the witness list assignments
* `ListColoring.ValidArms`, `ListColoring.GoodArms` : normalized shapes, and Rubin's shape

## Main results

* `ListColoring.gAdjB_triple`, `ListColoring.choosable_gtheta_triple_iff` : **three arms agree with
  `ListColoring.thetaGen`**, so the three-arm classification transfers instead of being redone
* `ListColoring.gArmBlocks` : **the forced chain along a blocking arm**, for any arity
* `ListColoring.gcol_gBadLists_eq_zero` : **blocking every branch pair leaves no coloring**
* `ListColoring.not_choosable_two_gtheta_of_four` : **no generalized theta with `n ≥ 4` arms is
  `2`-choosable**
* `ListColoring.choosable_two_gtheta_iff` : **the classification**, for every `n ≥ 3`
* `ListColoring.not_choosable_two_K24` : `K_{2,4}` is not `2`-choosable, via the arity-`4` case
-/
...
of `ListColoring.armStepB`, whose interiors start at the running offsets `0`, `k₁ - 1`,
`(k₁ - 1) + (k₂ - 1)`, … . -/
...
disjoint paths, of edge-lengths `k₁, …, k_n`. Compare `ListColoring.thetaGen a b c`, which is the
three-arm case on the very same layout (`ListColoring.gAdjB_triple`). -/
def gtheta (ks : List ℕ) : SimpleGraph (GTV ks) where
  Adj x y := x ≠ y ∧ (gAdjB ks x.val y.val ∨ gAdjB ks y.val x.val)
  symm := ⟨fun _ _ h => ⟨h.1.symm, h.2.symm⟩⟩
  loopless := ⟨fun _ h => h.1 rfl⟩

/-- Adjacency in `ListColoring.gtheta` is decidable, since it is read off a Boolean. -/
...
`armStepB` lemmas of `ListColoring.ThetaClass`. -/
...
/-! ### Three arms: agreement with `ListColoring.thetaGen`

`ListColoring.thetaGen a b c` uses the very same layout, so the two adjacency Booleans are equal on
...
`ListColoring.ThetaClass` is inherited rather than reproved. -/
...
/-- **The adjacencies agree**: `Θ(a, b, c)` in the general model *is* `ListColoring.thetaGen a b c`,
index for index. -/
...
`j - 1` and `j`. Compare `ListColoring.armBlockLists`, which does the same job for three arms with
...
length that turns `ListColoring.gArmLists` from a witness into a proof; it is presentation, not
mathematics, and mirrors `ListColoring.armBlockLists_forced` with the auxiliary colors moved out of
...
/-- Every list of `ListColoring.gArmLists` has two colors, as soon as the two branch colors are
...
`ListColoring.gListsAux` walks the arms with a running offset to find which arm an index belongs
...
`ListColoring.gBadListAt`, which peels off the two branch vertices first. -/
...
/-- Every list produced by `ListColoring.gListsAux` has two colors, provided `R`'s do. -/
...
juggling is needed — in contrast with three arms, where `ListColoring.ThetaClass` must play the
first arm's parity against the other two. Two role assignments cover every valid shape:

* `ListColoring.roleA`, used when all of the first four arms have length `≥ 2`: the branch lists
  `{1, 2}` and `{3, 4}` are disjoint, and the four arms block the four pairs one apiece;
* `ListColoring.roleB`, used when the first arm is a single edge: the branch lists `{1, 2}` and
...
/-- `ListColoring.roleA` is a `2`-list assignment: its four blocking pairs are drawn from
...
/-- `ListColoring.roleB` is a `2`-list assignment: its three blocking pairs `(1,2)`, `(1,3)`,
...
Sorting the arms costs nothing and, as in `ListColoring.ValidShape`, lets "at most one arm of length
...
/-- A normalized three-arm shape, from the three inequalities of `ListColoring.ValidShape`. -/
...
three-arm case is `ListColoring.choosable_two_thetaGen_iff`, transported along
`ListColoring.choosable_gtheta_triple_iff`; the arity-`≥ 4` case is
...
Every claim above was checked by brute force on concrete shapes before it was proved. The witness of
`ListColoring.gWitness` is `ListColoring.gBadLists` with `ListColoring.roleB` when the first arm is
a single edge and `ListColoring.roleA` otherwise, exactly as
`ListColoring.not_choosable_two_gtheta_of_four` chooses; `ListColoring.gWitnessCheck` verifies that
it is a `2`-list assignment with no coloring at all, and the sweep below runs it over *every* sorted
valid shape with four or five arms and total length at most `12`. -/

section Guards

/-- The witness `2`-list assignment used by `ListColoring.not_choosable_two_gtheta_of_four`. -/
...
four arms' single interior vertices. `ListColoring.not_choosable_two_gtheta_of_four` therefore
...
#print axioms ListColoring.gsize_take_succ
#print axioms ListColoring.gAdjAux_of_armStepB
#print axioms ListColoring.gAdjB_triple
#print axioms ListColoring.choosable_gtheta_triple_iff
#print axioms ListColoring.armBlocks_direct
#print axioms ListColoring.gArmBlocks
#print axioms ListColoring.card_gArmLists
#print axioms ListColoring.gListsAux_apply
#print axioms ListColoring.card_gBadLists
#print axioms ListColoring.gcol_gBadLists_eq_zero
#print axioms ListColoring.not_choosable_two_gtheta
#print axioms ListColoring.not_choosable_two_gtheta_of_four
#print axioms ListColoring.choosable_two_gtheta_iff
#print axioms ListColoring.K24Map_injective
#print axioms ListColoring.K24Map_adj
#print axioms ListColoring.not_choosable_two_K24
