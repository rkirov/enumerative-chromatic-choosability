...
`ListColoring.RubinHard` names `ListColoring.ThetaClassification`, the classification of
`2`-choosable *theta* graphs. This file **discharges it**:

> `θ_{a,b,c}` of a valid shape is `2`-choosable if and only if the shape is `(2, 2, \text{even})`.

The forward implication is proved here in the form `ListColoring.not_choosable_two_thetaGen`, and
`ListColoring.thetaClassification` is the resulting term of type `ListColoring.ThetaClassification`;
nothing is assumed.

## The argument

`ListColoring.thetaBadLists` gives both branch vertices the list `{1, 2}`, so a coloring of
...
* `ListColoring.const_block` — an arm whose interior lists are all `{1, 2}` forces its two ends to
...
* `ListColoring.armBlockLists_forced` — an arm with `m ≥ 2` interior vertices carrying the lists
  `ListColoring.armBlockLists α₀ β₀ m` propagates a forced chain: if its first interior vertex
  avoids `α₀` — which it must when `α = α₀` — then its last interior vertex is forced to the color
  `β₀`. Such an arm blocks the single pair `(α₀, β₀)`.
...
`θ_{a,b,c}` is read as a function `F : ℕ → ℕ`, the lists as `ListColoring.badListAt`, and the arms
as the three ranges of indices that `ListColoring.armStepB` links up.

## Main definitions

* `ListColoring.badListAt` : `ListColoring.thetaBadLists` as a function of the index of a vertex
* `ListColoring.thetaGenToTheta` : the embedding of `thetaGen 2 2 (2m)` into `ListColoring.theta m`

## Main results

* `ListColoring.alt_chain` : **the alternating chain** — a stretch of a path whose lists are all
  `{p, q}` alternates, and is determined by the color at either end
* `ListColoring.const_block` : **which pairs an arm with constant lists blocks**
* `ListColoring.armBlockLists_forced` : **the forced chain along a blocking arm**
* `ListColoring.card_thetaBadLists` : the witness is a `2`-list assignment, for every shape
* `ListColoring.col_thetaBadLists_eq_zero` : **the witness has no coloring at all**, for every valid
  non-Rubin shape
* `ListColoring.not_choosable_two_thetaGen` : **`θ_{a,b,c}` is not `2`-choosable** unless its shape
  is `(2, 2, \text{even})`
* `ListColoring.thetaClassification` : **`ListColoring.ThetaClassification`, discharged**
* `ListColoring.thetaGenToTheta` : the embedding of `thetaGen 2 2 (2m)` into `ListColoring.theta m`,
  and `ListColoring.choosable_two_thetaGen_of_goodShape` : **Rubin's own shape is `2`-choosable** on
  the general model
* `ListColoring.choosable_two_thetaGen_iff` : **the classification as an `↔`**
-/
...
sweep is the content of `ListColoring.const_block`: with the constant lists `{1, 2}` the theta graph
has a coloring exactly when its three arm lengths all have the same parity — otherwise it has an
odd cycle. The second extends the sweep of `ListColoring.thetaBadCheck` past the range recorded in
`ListColoring.RubinHard`. -/
...
This is the one induction behind both `ListColoring.const_block` and
`ListColoring.armBlockLists_forced`, and hence behind the whole classification. The stretch is
...
/-- **Constant lists alternate.** The special case of `ListColoring.alt_chain` in which every list
...
length that turns `ListColoring.armBlockLists` from a witness into a proof.
...
/-- `ListColoring.thetaBadLists` read as a function of the *index* of the vertex. The whole
analysis below runs on indices, `ListColoring.thetaBadLists_apply` being the (definitional) bridge
back to the vertices of `ListColoring.TGV`. -/
...
/-- Every list of `ListColoring.armBlockLists` has two colors, as soon as the two prescribed branch
colors are `1` or `2`. -/
...
/-- **`θ_{a,b,c}` has no coloring from `ListColoring.thetaBadLists`**, for every valid shape other
...
`(2, 2, \text{even})`: the witness `ListColoring.thetaBadLists` is a `2`-list assignment with no
coloring at all. -/
...
`ListColoring.ThetaClassification`. -/
...
`ListColoring.theta m` — a path of length `2m` with two extra vertices, each joined to both of its
ends — is `2`-choosable, and `ListColoring.thetaGen 2 2 (2 * m)` is the same graph on
...
/-- The position along the long path of `ListColoring.theta m` taken by the vertex of `θ_{2,2,2m}`
...
/-- **The embedding of `thetaGen 2 2 (2m)` into `ListColoring.theta m`.** It is in fact a
bijection — the two graphs are the same theta graph on differently named vertices — but only
injectivity and edge preservation are needed, since `SimpleGraph.Choosable.comap` transports
choosability backwards along such a map. -/
...
`ListColoring.theta m`, and `ListColoring.choosable_theta` applies. -/
...
The general theorems specialize to the explicit cases proved by hand in `ListColoring.RubinHard`,
...
/-- The family `θ_{2,2,\text{odd}}`, which `ListColoring.not_choosable_two_thetaOf_odd` settles on
the model `ListColoring.thetaOf`, is now available on `ListColoring.thetaGen` as well. -/
...
#print axioms ListColoring.armBlockLists_forced
#print axioms ListColoring.const_block
#print axioms ListColoring.card_armBlockLists
#print axioms ListColoring.thetaBadLists_apply
#print axioms ListColoring.card_thetaBadLists
#print axioms ListColoring.col_thetaBadLists_eq_zero
#print axioms ListColoring.not_choosable_two_thetaGen
#print axioms ListColoring.thetaGenToTheta_injective
#print axioms ListColoring.thetaGenToTheta_adj
#print axioms ListColoring.choosable_two_thetaGen_two_two
#print axioms ListColoring.choosable_two_thetaGen_of_goodShape
#print axioms ListColoring.choosable_two_thetaGen_iff
#print axioms ListColoring.thetaClassification
