...
The statement usually quoted — "two cycles meeting in at most one vertex" — is stated inside a proof
about a **connected** graph, and connectivity is load-bearing. Two *disjoint* even cycles with
nothing joining them are `2`-choosable: each even cycle is
(`ListColoring.choosable_two_closePath_of_odd`) and colourings of a disjoint union are independent.
Numerically, `C₄ ⊔ C₄` has four colourings from the constant list `{1, 2}`. So the configuration
formalized here is the **dumbbell**: a cycle `A`, a cycle `B`, and a path from a vertex of `A` to a
vertex of `B` meeting them only at its ends. The path is allowed to have length `0`, which is the
figure-eight — two cycles sharing exactly one vertex. This is what a cut vertex in a connected graph
of minimum degree `≥ 2` actually hands you.
...
sweep recorded in the module docstring of `ListColoring.RubinHard` for the analogous theta witness).

Everything rests on one observation, `ListColoring.forced_chain`. Along a walk `u 0, u 1, …, u m`,
...
`c m = c 0` yields a contradiction — this is `ListColoring.no_coloring_of_cycle_chain`, and it kills
...
The colours are laid out by `ListColoring.armCol α β m`: `α` at index `0`, `β` at index `m`, and
...
* `ListColoring.forced_chain`, `ListColoring.no_coloring_of_cycle_chain` : the forcing mechanism
* `ListColoring.not_choosable_two_of_dumbbell` : **the main result** — two cycles joined by a path,
  otherwise disjoint, are not `2`-choosable
* `ListColoring.not_choosable_two_of_figureEight` : the `l = 0` case, two cycles sharing one vertex
* `ListColoring.not_choosable_two_of_odd_closed_walk` : the odd-cycle companion, restated in the
...
* **Step 1, the core reduction.** `ListColoring.choosable_iff_of_coreIs` (`ListColoring.Theorem2`) —
  `G` is `n`-choosable iff its core is, for `n ≥ 2` — with
  `SimpleGraph.choosable_pendantTower_iff` (`ListColoring.Choosable`) underneath it. Both are
  unconditional.
* **The odd-cycle obstruction.** `ListColoring.not_colorable_two_of_hasOddCycle` and
  `ListColoring.hasOddCycle_of_not_colorable_two` (`ListColoring.Theorem2`), and
  `ListColoring.not_choosable_two_of_contains_odd_cycle` (`ListColoring.RubinHard`).
  `ListColoring.not_choosable_two_of_odd_closed_walk` below is a convenience restatement for callers
  holding a bare indexed closed walk rather than an embedded `ListColoring.closePath`; it claims
  nothing new.

`ListColoring.not_choosable_of_contains` transfers any of these obstructions to a supergraph.
-/
...
/-- **A cycle of forced colours has no colouring.** With the lists of `ListColoring.forced_chain`
...
differ, which is exactly what `ListColoring.forced_chain` needs.
...
`{c i, c (i+1)}`, which is what `ListColoring.forced_chain` consumes. -/
...
walking along three sequences of them. `ListColoring.ofSeq` transports lists from indices to
...
`ListColoring.not_colorable_two_of_hasOddCycle` (`ListColoring.Theorem2`) and as
`ListColoring.not_choosable_two_of_contains_odd_cycle` (`ListColoring.RubinHard`). Both of those
want an embedded copy of `ListColoring.closePath`, which costs an injectivity obligation; this
restatement takes a bare indexed closed walk, matching the interface of
`ListColoring.not_choosable_two_of_dumbbell` below, and is proved directly from
`ListColoring.no_coloring_of_cycle_chain` in five lines. Convenience, not mathematics. -/
...
`ListColoring.not_choosable_two_of_dumbbell`: cycles `A 0, …, A m, A 0` and `B 0, …, B n, B 0` with
...
The witness of `ListColoring.not_choosable_two_of_dumbbell` was checked by brute force before it was
...
#print axioms ListColoring.forced_chain
#print axioms ListColoring.no_coloring_of_cycle_chain
#print axioms ListColoring.not_choosable_two_of_odd_closed_walk
#print axioms ListColoring.not_choosable_two_of_dumbbell
#print axioms ListColoring.not_choosable_two_of_figureEight
